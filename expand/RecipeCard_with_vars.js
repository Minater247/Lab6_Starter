...
class RecipeCard extends HTMLElement {
	...
	constructor() {
		...
		// A4. Update styles to use CSS variables
		style.textContent = `
			* {
				font-family: var(--recipe-font-family, sans-serif);
				margin: 0;
				padding: 0;
			}
			a {
			...
			article {
				align-items: center;
				border: 1px solid var(--recipe-border-color, rgb(223, 225, 229));
				border-radius: 8px;
				display: grid;
			...
			p.organization {
				color: var(--recipe-org-color, black) !important;
			}
			p.title {
				display: -webkit-box;
				font-size: 16px;
			...
			span,
			time {
				color: var(--recipe-text-color, #70757A);
				font-size: 12px;
			}`;
